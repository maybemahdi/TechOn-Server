...
const createOrderFromCart = async (
  ...
  const productRows = payload.products
    ...
    <tr>
      ...
      <td style="padding: 10px; border-bottom: 1px solid #e2e8f0;">$${item?.price?.toFixed(
        2
      )}</td>
    </tr>
  `
          ...
          >
            ...
            <tr>
              <td style="padding: 24px; color: #333333;">
                <p style="font-size: 16px; margin: 0 0 8px 0;">Hello <strong>${
                  user.name
                }</strong>,</p>
...
const createSingleOrder = async (userId: string, payload: any) => {
  ...
  const price =
    ((product.priceStorage as any)!.find(
      (item: { storage: string; price: number }) =>
        item.storage === payload.storage
    )?.price || 0) * payload.quantity;
  const order = await prisma.order.create({
    data: {
      userId,
      paymentMethod: payload.paymentMethod,
  ...
  <body style="margin:0;padding:0;font-family:Arial,Helvetica,sans-serif;background-color:#f5f5f5;">
    <table role="presentation" cellspacing="0" cellpadding="0" border="0" width="100%">
      ...
      <td style="padding: 10px; border-bottom: 1px solid #e2e8f0;">$${price?.toFixed(
        2
      )}</td>
    </tr>
                  </tbody>
...
const overView = async (year: number) => {
  ...
  const monthly = Array.from({ length: 12 }, (_, i) => {
    const monthlyOrders = orders.filter((b) => b.createdAt.getUTCMonth() === i);

    return {
      month: dayjs().month(i).format("MMM"),
      count: monthlyOrders.reduce(
